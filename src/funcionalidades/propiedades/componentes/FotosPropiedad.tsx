...
import { comprimirImagen } from "@/funcionalidades/propiedades/utilidades/comprimirImagen";
...
import {
  FotoPropiedadItem,
  type ManejadoresManija,
} from "@/funcionalidades/propiedades/componentes/FotoPropiedadItem";
...
// Order visual de las fotos que NO se están arrastrando: la arrastrada
...
export function FotosPropiedad({ propiedadId, imagenesIniciales }: FotosPropiedadProps) {
  ...
  const [imagenesOptimistas, marcarOrdenOptimista] = useOptimistic(
    imagenes,
    (_estado: ImagenPropiedad[], nuevoOrden: ImagenPropiedad[]) => nuevoOrden,
  );
  ...
  function aplicarNuevoOrden(nuevoOrden: ImagenPropiedad[]) {
    setErrorOrden(null);
    ...
    iniciarTransicionOrden(async () => {
      ...
      const resultado = await reordenarImagenes(
        propiedadId,
        nuevoOrden.map((imagen) => imagen.id),
      );
  ...
  function moverConTeclado(indice: number, direccion: -1 | 1) {
    ...
    setAnuncioOrden(`Foto movida a la posición ${nuevoIndice + 1} de ${nuevoOrden.length}.`);
  }
  ...
  function terminarArrastre(evento: ReactPointerEvent<HTMLButtonElement>) {
    ...
    // sigue siendo válido para sacar la foto de ahí.
    const nuevoOrden = [...imagenesOptimistas];
  ...
  async function procesarArchivosSeleccionados(archivos: File[]) {
    ...
    if (archivos.length > disponibles) {
      setErrorLimite(
        `Podés cargar hasta ${LIMITE_IMAGENES} fotos por propiedad. Ya tenés ${imagenes.length} y elegiste ${archivos.length}.`,
      );
      return;
    }
    ...
    for (let indice = 0; indice < archivos.length; indice++) {
      ...
      try {
        ...
        const rutaArchivo = `${propiedadId}/${crypto.randomUUID()}.${extension}`;
        ...
        if (errorSubida) {
          erroresDeEstaTanda.push({ archivo: archivo.name, mensaje: "No se pudo subir la foto." });
          continue;
        }

        const resultado = await registrarImagen(propiedadId, rutaArchivo);
        ...
        erroresDeEstaTanda.push({
          ...
          mensaje: error instanceof Error ? error.message : "No se pudo procesar la foto.",
        });
      }
    }
  ...
  const estiloArrastreCard: CSSProperties = {
    ...
    // quién se está arrastrando — por eso antes dependía de la dirección.
    zIndex: 50,
  };
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      >
        <span className="font-medium text-negro">Agregar fotos</span>
        <span>Arrastrá las fotos acá o tocá para elegirlas.</span>
        <input
          type="file"
          accept="image/*"
          multiple
      ...
      {erroresSubida.length > 0 && (
        <div
          ...
          className="border-l-4 border-negro bg-gris-100 px-3 py-2 text-sm font-medium text-negro"
        >
          <p>No se pudieron subir algunas fotos:</p>
      ...
      {imagenesOptimistas.length === 0 ? (
        <p className="text-sm text-gris-600">
          Todavía no cargaste fotos. La primera va a ser la portada.
        </p>
      ) : (
